...
\begin{DoxyEnumerate}
\item Strong Scaling\+: Fixed problem size, varying core count
\begin{DoxyItemize}
\item Speedup\+: S(p) = T(1) / T(p)
\item Efficiency\+: E(p) = S(p) / p = T(1) / (p \texorpdfstring{$\ast$}{*} T(p))
\end{DoxyItemize}
\item Weak Scaling\+: Problem size scales with core count (constant work per core)
\begin{DoxyItemize}
\item Efficiency\+: E(p) = T(1) / T(p) (ideal = 1.\+0)
\end{DoxyItemize}
\end{DoxyEnumerate}

Usage\+: mpirun -\/np $<$\+N$>$ ./scaling\+\_\+benchmark \mbox{[}options\mbox{]}

Options\+: --strong Run strong scaling test only --weak Run weak scaling test only --min-\/ref $<$n$>$ Minimum refinements (default\+: 3) --max-\/ref $<$n$>$ Maximum refinements (default\+: 7) --degree $<$n$>$ Polynomial degree (default\+: 2) --output $<$file$>$ Output CSV file prefix (default\+: scaling\+\_\+results) --threads $<$n$>$ Threads per MPI process (default\+: auto) --trials $<$n$>$ Number of trials for averaging (default\+: 3) --warmup $<$n$>$ Number of warmup runs (default\+: 1) --dim $<$n$>$ Dimension 2 or 3 (default\+: 2) 
