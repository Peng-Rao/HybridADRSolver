...
\begin{titlingpage}
    \begin{center}
        ...
        {\Large{\textbf{Hybrid Thread-MPI Parallelization for the ADR Equation}}}
        \rule{0.95\textwidth}{0.7mm}
...
\cref{fig:strong_scaling} presents the strong scaling results for both solver types. The total execution time (\cref{fig:total}) demonstrates that both solvers achieve reasonable scaling up to 28 processors, with the matrix-free approach consistently outperforming the matrix-based solver across all problem sizes and processor configurations. For the largest problem (16.8M DoFs), the matrix-free solver achieves a total execution time of 7.2~s on 28 cores compared to 13.5~s for the matrix-based approach, representing a $1.9\times$ speedup.

The time decomposition reveals distinct scaling behaviors for different computational phases. The setup and assembly phase (\cref{fig:setup}) exhibits excellent scaling for both solvers, as these operations are largely embarrassingly parallel. Notably, the matrix-free solver demonstrates significantly lower assembly times due to its on-the-fly computation strategy, which avoids explicit matrix storage. The solve phase (\cref{fig:solve}) dominates the total execution time and exhibits good scaling up to 14 processors, with diminishing returns beyond this point due to increased communication overhead relative to local computation.

\begin{figure}[htbp]
    \centering
    % Total time - full width at top
    \subfloat[Total time\label{fig:total}]{%
        \includegraphics[width=0.9\textwidth]{figures/strong_scaling_total.pdf}%
    }
    ...
    \caption{Strong scaling results for both solver types. The dashed line indicates ideal linear scaling.}
    \label{fig:strong_scaling}
\end{figure}

\subsubsection{Parallel Efficiency}

\cref{fig:parallel_efficiency} illustrates the parallel efficiency for both solver types. The matrix-free solver (\cref{fig:efficiency_mf}) maintains higher efficiency across all processor counts, achieving approximately 74\% efficiency at 28 cores for the 16.8M DoF problem. The matrix-based solver (\cref{fig:efficiency_mb}) exhibits lower efficiency, dropping to approximately 50\% at 28 cores. This difference is attributed to the matrix-free approach's superior computation-to-communication ratio and reduced memory bandwidth requirements.
...
A comparative analysis of pure MPI versus pure threading parallelization reveals a striking performance disparity, as illustrated in \cref{fig:mpi_vs_thread}. At 28 cores with 16.8M DoFs, pure MPI (28 processes $\times$ 1 thread) achieves an execution time of 7.2~s for the matrix-free solver, while pure threading (1 process $\times$ 28 threads) requires 281.8~s---representing a $39\times$ performance degradation.

The time breakdown (\cref{fig:mpi_thread_breakdown}) reveals that all computational phases suffer under threading, with the solve phase experiencing the most significant absolute slowdown. The performance ratio (\cref{fig:mpi_thread_ratio}) demonstrates that this gap widens dramatically as the core count increases, indicating poor thread scalability. The memory locality analysis (\cref{fig:mpi_thread_memory}) illustrates the fundamental issue: threading forces all 16.8M DoFs into a single shared memory space, while MPI distributes approximately 600K DoFs per process, enabling efficient cache utilization. Consequently, the parallel efficiency (\cref{fig:mpi_thread_efficiency}) for threading collapses to less than 2\% at 28 cores, compared to 74\% for MPI.

This disparity can be attributed to several factors:

\begin{itemize}
    \item \textbf{Memory bandwidth saturation}: Threading confines all 28 threads to a single memory controller, creating a bottleneck, whereas MPI processes can utilize multiple memory controllers concurrently.
    \item \textbf{NUMA effects}: Threads accessing remote NUMA nodes incur 3--4$\times$ memory latency penalties, while MPI processes can be pinned to local NUMA nodes for optimal memory access patterns.
    \item \textbf{Cache efficiency}: With threading, the entire 16.8M DoF problem resides in a shared memory space, causing frequent cache misses. MPI distributes approximately 600K DoFs per process, enabling superior cache utilization.
    \item \textbf{Library optimization}: The underlying libraries (deal.II, PETSc) are primarily optimized for distributed memory parallelism via MPI, with thread-level parallelism serving as a secondary optimization.
\end{itemize}

Experiments with hybrid configurations (MPI $\times$ threads) confirm that maximizing the number of MPI processes yields optimal performance. For example, at 28 total cores, the configuration 28M$\times$1T (pure MPI) outperforms 1M$\times$28T (pure threading) by more than an order of magnitude for both solver types.
...
\begin{figure}[htbp]
    ...
    \caption{Comparison of pure MPI versus pure threading parallelization strategies for 16.8M DoFs: (a) time breakdown showing that all phases are slower with threading, (b) performance ratio demonstrating the widening gap at higher core counts, (c) DoFs per MPI process illustrating memory locality advantages, and (d) parallel efficiency comparison.}
    \label{fig:mpi_vs_thread}
\end{figure}

\subsubsection{Scalability Summary}

The scalability analysis demonstrates that the matrix-free solver offers superior performance and parallel efficiency compared to the matrix-based approach. Pure MPI parallelization significantly outperforms threading due to better memory locality, reduced bandwidth contention, and optimized library support. When hybrid configurations are employed, they should favor higher MPI process counts with minimal threading. For production deployments, we recommend pure MPI parallelization with one process per physical core to maximize performance.


\section{Conclusion}

This work presents a comprehensive study of hybrid MPI+threading parallel implementations for solving the steady-state Advection-Diffusion-Reaction equation using finite element methods. Two fundamentally different solver approaches were implemented, analyzed, and compared:

\begin{itemize}
    \item \textbf{Matrix-based solver}: Explicitly assembles the sparse system matrix using the \texttt{WorkStream} framework for thread-parallel assembly, coupled with Algebraic Multigrid (AMG) preconditioning via PETSc for the iterative solve phase.
    \item \textbf{Matrix-free solver}: Computes matrix-vector products on-the-fly using sum factorization and SIMD vectorization techniques, with Geometric Multigrid (GMG) preconditioning employing Chebyshev polynomial smoothers.
\end{itemize}

\subsection*{Correctness Verification}

Both implementations achieve the expected optimal convergence rates ($\mathcal{O}(h^{k+1})$ in the $L^2$ norm for polynomial degree $k$), validating the correctness of the discretization and solver implementations against the manufactured solution benchmark.

\subsection*{Performance Comparison}

The comprehensive performance analysis reveals several key findings:

\begin{enumerate}
    \item \textbf{Memory efficiency}: The matrix-free solver achieves a $6.3\times$ reduction in memory footprint compared to the matrix-based approach at the largest problem size (268M DoFs), consuming only 4.1~GB versus 26~GB. This advantage enables the solution of significantly larger problems on equivalent hardware.

    \item \textbf{Algorithmic scalability}: The GMG-preconditioned matrix-free solver exhibits ideal $\mathcal{O}(N)$ complexity, maintaining a constant iteration count of 6 across all problem sizes from $10^3$ to $2.68 \times 10^8$ DoFs. In contrast, the AMG-preconditioned matrix-based solver shows a gradual increase in iterations (19 to 48) as problem size grows.

    \item \textbf{Crossover behavior}: While the matrix-based solver demonstrates slightly faster execution for small problems due to lower setup overhead, the matrix-free solver achieves superior performance beyond a crossover point of approximately $N_{\text{dof}} \approx 263{,}000$. At the largest scale, the matrix-free approach is $1.4\times$ faster in total execution time.

    \item \textbf{Parallel efficiency}: The matrix-free solver maintains approximately 74\% parallel efficiency at 28 cores for 16.8M DoFs, compared to 50\% for the matrix-based solver. This superior efficiency stems from the reduced memory bandwidth requirements and improved computation-to-communication ratio.
\end{enumerate}

\subsection*{Parallelization Strategy Insights}

A critical finding of this study concerns the relative effectiveness of different parallelization paradigms:

\begin{itemize}
    \item \textbf{Pure MPI dominance}: Pure MPI parallelization dramatically outperforms pure threading, achieving up to $39\times$ better performance at 28 cores. This disparity arises from memory bandwidth saturation, NUMA effects, cache inefficiency, and the MPI-centric optimization of underlying libraries.

    \item \textbf{Hybrid configuration guidelines}: When hybrid MPI+threading configurations are employed, maximizing the number of MPI processes while minimizing threads per process yields optimal results. Configurations favoring threading (e.g., 1M$\times$28T) should be avoided in favor of MPI-dominant configurations (e.g., 28M$\times$1T or 14M$\times$2T).
\end{itemize}

\subsection*{Recommendations}

Based on these findings, we offer the following practical recommendations:

\begin{enumerate}
    \item For large-scale simulations ($N_{\text{dof}} > 250{,}000$), the matrix-free solver with GMG preconditioning should be preferred due to its superior memory efficiency, algorithmic scalability, and parallel performance.

    \item For smaller problems or cases where rapid prototyping is prioritized, the matrix-based solver remains competitive and may offer simpler debugging and analysis.

    \item Production runs should employ pure MPI parallelization with one process per physical core. If hybrid parallelization is necessary (e.g., due to memory constraints), configurations with high MPI-to-thread ratios should be adopted.

    \item Memory-limited scenarios benefit substantially from the matrix-free approach, which can solve problems $6\times$ larger than the matrix-based method on identical hardware.
\end{enumerate}
...
\begin{itemize}
    \item Extension to three-dimensional problems and higher polynomial degrees, where the advantages of matrix-free methods are expected to be even more pronounced.
    \item Investigation of GPU acceleration for the matrix-free operator evaluation, leveraging the inherent parallelism of the element-wise computations.
    \item Adaptive mesh refinement strategies combined with the matrix-free framework for problems with localized features.
    \item Application to time-dependent and nonlinear variants of the ADR equation.
\end{itemize}

In summary, this work demonstrates that matrix-free methods with geometric multigrid preconditioning, combined with MPI-based distributed memory parallelization, represent the optimal approach for large-scale finite element simulations of advection-diffusion-reaction problems on modern HPC architectures.
