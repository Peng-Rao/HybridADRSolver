...
It models how a scalar quantity u (such as concentration, temperature, or chemical potential) is distributed within a domain due to three competing physical processes: diffusion, advection, and reaction.

\subsection{Weak formulation}
First of all, we define the trial function space and test function space. For trial space, solution $u$ belongs to the Sobolev space $H^1(\Omega)$ with Dirichlet boundary conditions incorporated:
\begin{equation}
    V_g = \{ u \in H^1(\Omega) : u = g \text{ on } \Gamma_D \}.
\end{equation}
For test space, the test function $v$ belongs to the Sobolev space $H^1(\Omega)$ with homogeneous Dirichlet boundary conditions:
\begin{equation}
    V_0 = \{ v \in H^1(\Omega) : v = 0 \text{ on } \Gamma_D \}.
\end{equation}
Multiply the governing equation by a test function $v \in V_0$ and integrate over the domain $\Omega$:
\begin{equation}
    \int_{\Omega} \left( -\nabla \cdot (\mu \nabla u) + \nabla \cdot (\boldsymbol{\beta} u) + \gamma u \right) v \, dx = \int_{\Omega} f v \, dx.
\end{equation}
Using the linearity of the integral, we separate the terms:
\begin{equation}
    -\int_{\Omega} \nabla \cdot (\mu \nabla u) v \, dx + \int_{\Omega} \nabla \cdot (\boldsymbol{\beta} u) v \, dx + \int_{\Omega} \gamma u v \, dx = \int_{\Omega} f v \, dx.
\end{equation}
...
\end{equation}
Split the boundary integral into Dirichlet ($\Gamma_D$) and Neumann ($\Gamma_N$) parts:
\begin{equation}
    \int_{\partial \Omega} \mu (\nabla u \cdot \mathbf{n}) v \, ds = \int_{\Gamma_D} \mu (\nabla u \cdot \mathbf{n}) v \, ds + \int_{\Gamma_N} \mu (\nabla u \cdot \mathbf{n}) v \, ds.
\end{equation}
Since $v = 0$ on $\Gamma_D$, the first term vanishes. On $\Gamma_N$, we use the Neumann condition $\nabla u \cdot \mathbf{n} = h$:
\begin{equation}
    \int_{\Gamma_N} \mu (\underbrace{\nabla u \cdot \mathbf{n}}_{h}) v \, ds = \int_{\Gamma_N} \mu h v \, ds.
\end{equation}
Find $u \in S_g$ such that for all $v \in V$:
\begin{equation}
    \int_{\Omega} \mu \nabla u \cdot \nabla v \, dx + \int_{\Omega} \nabla \cdot (\boldsymbol{\beta} u) v \, dx + \int_{\Omega} \gamma u v \, dx = \int_{\Omega} f v \, dx + \int_{\Gamma_N} \mu h v \, ds
\end{equation}
This can be written in the abstract form $a(u,v) = L(v)$ where:
\begin{align}
    a(u,v) & = \int_{\Omega} (\mu \nabla u \cdot \nabla v + \nabla \cdot (\boldsymbol{\beta} u) v + \gamma u v) \, dx, \\
    L(v)   & = \int_{\Omega} f v \, dx + \int_{\Gamma_N} \mu h v \, ds.
\end{align}
Since $g$ may not be in the test space $V_0$, we introduce a lifting function $u_g \in V_g$ such that $u_g = g$ on $\Gamma_D$. We can express the solution as $u = u_0 + u_g$ where $u_0 \in V_0$. The weak formulation then becomes:
\begin{equation}
    \text{Find $u_0 \in V_0$ such that for all $v \in V_0$:} \quad
    a(u_0, v) = L(v) - a(u_g, v).
\end{equation}
...
The problem domain boundary $\partial \Omega$ is split into Dirichlet ($\Gamma_D$) and Neumann ($\Gamma_N$) portions to test mixed boundary conditions.
...
To solve the weak formulation (\cref{sec:problem_statement}) numerically, we employ the Finite Element Method (FEM). This involves approximating the infinite-dimensional function spaces $V_g$ and $V_0$ with finite-dimensional subspaces defined on a computational mesh.
...
\subsection{Treatment of Advection Dominance}
It is well known that for convection-dominated problems (where the Péclet number $Pe = \frac{|\boldsymbol{\beta}|h}{2\mu} > 1$), the standard Galerkin formulation described above may produce spurious non-physical oscillations. While the manufactured solution chosen in \cref{sec:numerical_verification} allows for convergence testing, typical industrial applications often require stabilization techniques such as Streamline-Upwind Petrov-Galerkin (SUPG) to ensure robust solutions. For this verification benchmark, we utilize a sufficiently refined mesh to maintain stability within the standard Galerkin framework.
