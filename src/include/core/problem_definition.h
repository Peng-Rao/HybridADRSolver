 ...
 * @brief Defines the problem interface and a concrete
 * Advection-Diffusion-Reaction (ADR) problem.
 *
 * This file contains the abstract base class `ProblemInterface` which allows
 * the solver to be decoupled from specific physics. It also provides a concrete
 * implementation `ADRProblem` using the Method of Manufactured Solutions (MMS)
 * for verification.
 */
 ...
 *
 * This class defines the necessary interfaces for coefficients (diffusion,
 * advection, reaction), source terms, boundary conditions, and exact solutions.
 * The solver relies solely on this interface, allowing for dependency
 * inversion.
 *
...
public:
    ...
    /**
     * @brief Returns the advection velocity field \f$
     * \mathbf{\beta}(\mathbf{x}) \f$.
     */
    ...
    /**
     * @brief Returns the set of boundary IDs where Dirichlet conditions are
     * applied.
     */
    ...
    /**
     * @brief Returns the set of boundary IDs where Neumann conditions are
     * applied.
     */
    [[nodiscard]] virtual std::set<types::boundary_id>
    ...
    /**
     * @brief Gets the function describing the Dirichlet boundary value \f$
     * g_D(\mathbf{x}) \f$ for a specific boundary ID.
     */
     ...
     * @return true if symmetric (e.g., pure diffusion), false otherwise (e.g.,
     * with advection).
     */
...
/**
 * @brief Manufactured exact solution: \f$ u(\mathbf{x}) = \prod \sin(\pi x_i)
 * \f$.
 *
 * This function is used to verify the solver accuracy. It satisfies homogeneous
 * Dirichlet boundary conditions on the unit hypercube (except where we manually
 * apply Neumann).
 */
 ...
 *
 * Calculates \f$ \nabla u \cdot \mathbf{n} \f$ at x=1, where \f$ \mathbf{n} =
 * (1, 0, \dots)^T \f$. Since \f$ \cos(\pi) = -1 \f$, the flux is negative.
 */
...
/**
 * @brief Concrete implementation of a steady-state Advection-Diffusion-Reaction
 * problem.
 *
...
public:
    ...
    /**
     * @brief Computes the source term \f$ f \f$ using the Method of
