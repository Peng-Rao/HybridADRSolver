...
def plot_weak_scaling(df, output_dir):
    ...
    for solver in ["matrix_based", "matrix_free"]:
        ...
        if not solver_df.empty and len(solver_df) > 0:
            t1 = solver_df[solver_df["n_mpi"] == solver_df["n_mpi"].min()][
                time_col
            ].values[0]
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "weak_scaling.png"), dpi=150, bbox_inches="tight"
    )
...
def plot_memory_comparison(df, output_dir):
    ...
    if "matrix_based" in grouped.columns and "matrix_free" in grouped.columns:
        ...
        ratio = ratio.dropna()
        ax2.bar(
            range(len(ratio)), ratio.values, color=COLORS["matrix_based"], alpha=0.7
        )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "memory_comparison.png"), dpi=150, bbox_inches="tight"
    )
...
def main():
    ...
    parser.add_argument(
        "--results-dir", default="results", help="Directory containing CSV results"
    )
    parser.add_argument(
